...
export default function ProductsPage() {
  ...
  return (
    <div className={`search-page-container row mx-0 justify-content-center  ${results.length > 0 ? 'py-5' : ''} ${loading === "LOADING_ENDED" && search.length > 0 && results.length == 0 ? 'py-5 align-content-start' : 'align-content-center'}`}>
      <div className="search-input-field col-12 col-sm-10 col-md-6">
        ...
        <div className="row mx-0 justify-content-center mt-3 px-0" style={{ height: '50px'}}>
            {loading === "LOADING_STARTED" && <p className='loading-label px-0'>Loading...</p>}
            {loading === "LOADING_ENDED" && search.length > 0 && (<span className='products-count-label px-0'>Total results count: <span className='products-count'>{results.length}</span></span>)}
            {loading === "LOADING_ENDED" && search.length > 0 && results.length == 0 ? <NoFoundData></NoFoundData>: ''}
        </div>
      </div>
